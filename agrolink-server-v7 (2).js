...
function formatPost(post) {
    ...
    // Alan adı normalizasyonu — UI userProfilePic/userName bekliyor
    p.userProfilePic = p.userProfilePic || p.profilePic || null;
    p.userName       = p.userName       || p.name       || 'Kullanıcı';
    p.userUsername   = p.userUsername   || p.authorUsername || p.username || '';
...
// ─── PUBLIC STATS — Ana sayfa için token gerektirmez ─────────────────
app.get('/api/public/stats', async (req, res) => {
    try {
        ...
        res.json({
            ...
            memberCount : parseInt(users?.count  || 0),
...
// ─── ANA SAYFA (Landing) ────────────────────────────────────────────
// Sunucudaki dosya: public/index.html (agro-sosyal.html)
app.get('/', (req, res) => {
...
// ─── PLATFORM (uygulama) — /agro-sosyal veya /agro-sosyal/ ──────────
// Sunucudaki dosya: public/agro-sosyal/index.html (agrolink-ui.html)
app.get(['/agro-sosyal', '/agro-sosyal/'], (req, res) => {
    ...
    if (fssync.existsSync(fallback)) return res.sendFile(fallback);
    res.status(404).json({ error: 'Platform sayfası bulunamadı. public/agro-sosyal/index.html dosyasını oluşturun.' });
});
